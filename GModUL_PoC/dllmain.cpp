...
void strafer(CUserCmd* user_cmd) {
	...
	auto flags = *reinterpret_cast<u32*>((uptr)local + 0xF8);
	auto velocity = *reinterpret_cast<Vector*>((uptr)local + 0x108);
	...
	if (!(flags & 0x1)) {
		...
		static auto cl_sidespeed = 175.f; //sidespeed_var.get_float();
...
bool __fastcall hooked_create_move(void* instance, [[maybe_unused]] void* edx, float sample_framerate, CUserCmd* user_cmd) {
	...
	if(settings.speedhack && GetAsyncKeyState('R') && !in_speedhack) {
		in_speedhack = true;
		using CL_Move = void(*)(float accumulated_extra_samples, bool bFinalTick);
		static auto move = reinterpret_cast<CL_Move>(signature::resolve_callgate(signature::find_pattern("engine", "E8 ? ? ? ? 83 C4 08 FF 15 ? ? ? ? DC 25")));
...
HRESULT STDMETHODCALLTYPE present(IDirect3DDevice9* thisptr, const RECT* src, const RECT* dest, HWND wnd_override, const RGNDATA* dirty_region) {
	...
	if(!init) {
		...
		if (menu) {
			...
			if(ImGui::Button("FixLoading Time")) {
				//ConvarWrapper kek("ui_loadingscreen_transition_time");
				//kek.set_value(0.f);
				constexpr uptr transition_offset_dll = 0xA12974;
				constexpr uptr transition_offset_0 = 0x1c;
				constexpr uptr transition_offset_1 = 0x2c;
				static auto client_base = reinterpret_cast<uptr>(signature::resolve_library("client"));
				static auto transition = reinterpret_cast<float*>(*reinterpret_cast<uptr*>(*reinterpret_cast<uptr*>(client_base + transition_offset_dll) + transition_offset_0) + transition_offset_1);
				*transition = 0.f;
			}
			ImGui::End();
		}
...
DWORD WINAPI main_thread(LPVOID lpArguments) {
	...
	engine = (engine_client*)create_interface_engine("VEngineClient015", NULL);
	cvar = (Cvar*)create_interface_vstdlib("VEngineCvar007", NULL);
	ent_list = (EntList*)create_interface_client("VClientEntityList003", NULL);

	static auto clientmode_func = client_mode_fn(signature::resolve_callgate(signature::find_pattern("client", "E8 ? ? ? ? 83 3E 01")));
	client_mode = clientmode_func();
	create_move_hook = std::make_unique<hooks::HookFunction<CClientMode, 0>>((CClientMode*)client_mode, 24, reinterpret_cast<void*>(&hooked_create_move));
